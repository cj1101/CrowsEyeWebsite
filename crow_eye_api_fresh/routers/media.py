...
async def list_media(
    ...
    **Features:**
    ...
    try:
        ...
        # Ensure media_data is a valid dictionary
        if not isinstance(media_data, dict):
            media_data = {"photos": [], "videos": [], "finished_posts": []}
        ...
        for category, paths in media_data.items():
            ...
            for path in paths:
                try:
                    ...
                    print(f"Error processing media item {path}: {item_error}")
                    continue
        ...
        # Return empty but valid response structure instead of raising exception
        print(f"Error in list_media: {str(e)}")
        return MediaSearchResponse(
            items=[],
            total=0,
            query=query,
